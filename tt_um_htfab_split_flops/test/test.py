...
def setup_clock(dut):
    ...
    async def single_phase_clock(cycles):
    ...
    async def two_phase_clock(cycles):
        for i in range(cycles):
            await Timer(10, units='ns')
            dut.clk2.value = 1
            await Timer(20, units='ns')
    ...
    if gate_level:
        dut.clk.value = 0
        dut.clk2.value = 0
        return two_phase_clock
    else:
        dut.clk.value = 0
        return single_phase_clock


@cocotb.test(skip=(not gate_level))
async def check_scanchain(dut):
    tick = setup_clock(dut)
    dut.rst_n.value = 0
    dut.ena.value = 1
    dut.buttons.value = 0
    dut.seg_invert.value = 0
    dut.scan_enable.value = 1
    dut.scan_data_in.value = 0

    random.seed(1)
    test_data = [random.randrange(2) for _ in range(1000)]
    send_buf = list(test_data)
    receive_buf = []
    max_steps = 2000
    for step, bit in enumerate(send_buf):
        assert step < max_steps
        dut.scan_data_in.value = bit
        await tick(1)
        res = dut.scan_data_out.value
        if res.is_resolvable:
            receive_buf.append(res)
        else:
            send_buf.append(0)
    assert receive_buf == test_data
